...
// This caused a compilation error and it was only needed once,
// so I replaced it with an inline computation --Dana Nau, 2022-2-15
// double abs(double x)
// {
// 	return x < 0?-x:x;
// };
...
public:
	direction() : myId(id++)
	{
		for (int i = 0; i < myId; ++i)
		{
			...
			// See my comment where abs is defined --Dana Nau, 2022-2-15
			// slewTimes.push_back(abs(a-b));
			slewTimes.push_back(a < b ? b - a : a - b);
		};
	};
	string name() const
	{
	...
	void init(ostream &o) const
	{
		...
		if (probType == TIMED || probType == COMPLEX || probType == NUMERIC)
		{
			...
			for (int i = 0; i < myId; ++i)
			{
				o << "\t(= (slew_time " << name() << " " << allDirections[i]->name() << ") " << slewTimes[i]
				  << ")\n";
				o << "\t(= (slew_time " << allDirections[i]->name() << " " << name() << ") " << slewTimes[i]
				  << ")\n";
			};
		};
	};
...
public:
	...
	void init(ostream &o) const
	{
		...
		{
			o << "\t(calibration_target instrument" << myID << " " << targets[i].name() << ")\n";
			if (probType == COMPLEX || probType == TIMED)
				o << "\t(= (calibration_time instrument" << myID << " " << targets[i].name() << ") " << calibrationTimes[i] << ")\n";
		};
	};
	void goal(ostream &o) const {};
};
...
public:
	satellite(int is) : myID(id++),
						d(selectOne<direction, target, observation>(problem::instance()->targets,
																	problem::instance()->observations)),
						end(selectOne<direction, target, observation>(problem::instance()->targets,
																	  problem::instance()->observations)),
						interesting(rnd(5) < 2),
						fuel(100 * (1 + rnd()))
	{
	...
	void init(ostream &o) const
	{
		...
		for (int i = 0; i < instruments.size(); ++i)
			o << "\t(on_board " << instruments[i].name() << " satellite" << myID << ")\n";
