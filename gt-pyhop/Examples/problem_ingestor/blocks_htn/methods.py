...
def has(collection: dict, key: str, val) -> bool:
    return key in collection.keys() and collection[key] == val
...
def status(b1, state, mgoal):
    ...
    if is_done(b1, state, mgoal):
        return "done"
    elif not has(state.clear, b1, True):
        return "inaccessible"
    elif not (b1 in mgoal.pos) or mgoal.pos[b1] == "table":
        return "move-to-table"
    elif is_done(mgoal.pos[b1], state, mgoal) and has(state.clear, mgoal.pos[b1], True):
